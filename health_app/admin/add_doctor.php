...
$nama = '';
$email = '';
// Default role untuk dokter adalah 'doctor'
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    $role = 'doctor'; // Role default untuk dokter
    // Jika Anda menambahkan kolom 'spesialisasi' di tabel users, Anda bisa mengambilnya di sini:
    ...
    if (empty($nama) || empty($email) || empty($password) || empty($confirm_password)) {
        ...
        if ($result_check->num_rows > 0) {
            ...
            // PERHATIAN: Query ini tidak menyertakan 'spesialisasi' karena Anda melaporkan kolom itu tidak ada.
            // Jika Anda menambahkan kolom 'spesialisasi' di masa mendatang, sesuaikan query ini dan bind_param.
            $stmt_insert = $conn->prepare("INSERT INTO users (nama, email, password, role) VALUES (?, ?, ?, ?)");
            if ($stmt_insert) {
                $stmt_insert->bind_param("ssss", $nama, $email, $hashed_password, $role);
                // Jika ada 'spesialisasi': $stmt_insert->bind_param("sssss", $nama, $email, $hashed_password, $role, $spesialisasi);
                ...
                if ($stmt_insert->execute()) {
                    ...
                    $error = "Gagal menambahkan dokter: " . $stmt_insert->error;
                }
                $stmt_insert->close();
            } else {
...
<body>
    ...
    <div class="container main-content-container">
        <h1 class="mb-5 text-center">Tambah Dokter Baru</h1>

        <div class="form-card">
            <div class="card-header">Isi Data Dokter</div>
            <div class="card-body">
                <?php if ($success) : ?>
                ...
                <form action="add_doctor.php" method="POST">
                    ...
                    <div class="d-flex justify-content-between mt-4">
                        ...
                        <button type="submit" class="btn btn-submit"><i class="fas fa-save me-2"></i> Tambah Dokter</button>
                    </div>
                </form>
            </div>
        </div>
